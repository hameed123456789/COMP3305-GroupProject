...
# Compute unique numeric keys for each FEN
# The key uses the FEN character sum plus the original row index
# so every key is unique even if different FENs have the same sum.
keys = [sum(ord(c) for c in fen) * len(data) + i for i, fen in enumerate(data)]
